...
func (data Data) extrapolatePastReading() int {
	result := 0
	for i := len(data.regression) - 2; i >= 0; i-- {
		result = data.regression[i][0] - result
	}
	return data.recording[0] - result
}
...
func main() {
	...
	for _, currentData := range result {
		sum += currentData.extrapolatePastReading()
	}
	fmt.Println(sum)
}
